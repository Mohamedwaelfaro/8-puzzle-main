...
class Puzzle:
    ...
    def setBlocksMatrix(self):
        ...
        for k in range(3):
            for j in range(3):
                blocks.append({'rect': pygame.Rect(
                    block_x, block_y, block_w, block_h), 'color': colors.BABY_BLUE, 'block': m[k][j]})
                block_x += block_w+1
                i += 1
            block_y += block_h+1
            block_x = self.x
    ...
    def setBlocks(self, string):
        ...
        if self.validNumbers(numbers):
            ...
            for k in range(3):
                for j in range(3):
                    blocks.append({'rect': pygame.Rect(
                        block_x, block_y, block_w, block_h), 'color': colors.BABY_BLUE, 'block': int(numbers[i])})
